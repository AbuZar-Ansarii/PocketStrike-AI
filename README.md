...
*   **Installed App & Wi-Fi Auditing**: Instantly inspect user app package names/APK locations, read/write to the Android system clipboard, scan nearby Wi-Fi hotspots for security profiles, and announce outputs out loud using native Text-To-Speech (TTS).
*   **Automated Shizuku Provisioning**: Auto-scans standard storage and download folders for exported client files, moves them to your Termux path, isolates executions from conflicting Termux dynamic linkers, and triggers the Android system authorization dialog automatically using TTY/PTY emulation on startup.
...
| 33 | `scan_wifi_networks()` | Scans nearby Wi-Fi hotspots and returns network details (SSID, BSSID, RSSI, channel, security). |
